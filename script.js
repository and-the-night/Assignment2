...
async function getWords(word, location) {
  let wordsArray = [];
  ...
  for (let i = 0; i < 5; i++) {
    const word = await askWord(newWord);
    wordsArray.push(word);
    newWord = word;
  }

  const wordsString = wordsArray.join("→");
  drawWord(wordsString, location);
  console.log(wordsString);
}

async function askWord(word) {
  let prompt =
    "return 5 words related to " + word + " with no extra words or punctuation";
  ...
  let textResponse = json_response.output.join("").trim().split(" ");
...
function initInterface() {
  ...
  inputBox.addEventListener("keydown", function (event) {
    ...
    if (event.key === "Enter") {
      ...
      getWords(inputValue, { x: inputLocationX, y: inputLocationY });
    }
  });
